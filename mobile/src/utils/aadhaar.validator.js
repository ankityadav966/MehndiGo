/**
 * Mobile Client Aadhaar Card Validator (UIDAI Standards & Verhoeff Checksum)
 */
...
export function checkVerhoeff(str) {
  let c = 0;
...
export function validateAadhaarNumber(aadhaarNumber) {
  if (!aadhaarNumber) {
    return { valid: false, cleanNumber: "", message: "Please enter your 12-digit Aadhaar Number" };
  }
  ...
  if (clean.startsWith("0") || clean.startsWith("1")) {
    return { valid: false, cleanNumber: clean, message: "Invalid Aadhaar: Aadhaar numbers cannot start with 0 or 1" };
  }

  // Reject repeating single digits (e.g., 222222222222, 999999999999)
  if (/^(\d)\1{11}$/.test(clean)) {
    return { valid: false, cleanNumber: clean, message: "Invalid Aadhaar: Repeating digit sequence is not allowed" };
  }

  // Reject known dummy sequences
  if (KNOWN_DUMMY_PATTERNS.includes(clean)) {
    return { valid: false, cleanNumber: clean, message: "Invalid Aadhaar: Please enter a genuine, valid 12-digit Aadhaar number" };
  }

  // Reject repeating blocks
  if (clean.slice(0, 4) === clean.slice(4, 8) && clean.slice(4, 8) === clean.slice(8, 12)) {
    return { valid: false, cleanNumber: clean, message: "Invalid Aadhaar: Repetitive block pattern is not allowed" };
  }

  // Verhoeff checksum algorithm validation
  const isVerhoeffValid = checkVerhoeff(clean);
  if (!isVerhoeffValid) {
    return { valid: false, cleanNumber: clean, message: "Invalid Aadhaar number checksum. Please check your card number." };
  }
 ...
 */
export function validateAadhaarPhotos(frontPhoto, backPhoto) {
  const front = String(frontPhoto || "").trim();
  const back = String(backPhoto || "").trim();

  if (!front) {
    return { valid: false, message: "Please upload Aadhaar card Front photo" };
  }

  if (!back) {
    return { valid: false, message: "Please upload Aadhaar card Back photo" };
  }

  // Front and Back photos must be distinct files / URIs
