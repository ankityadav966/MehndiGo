...
export default function BookingScreen() {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      // If payment is ONLINE, redirect to Razorpay payment screen / modal
      if (paymentMethod === "ONLINE") {
        const { createPaymentSession } = require("../services/payment");
        const bookingIdVal = res.data.id || res.data.bookingId;
        try {
          const sessionData = await createPaymentSession(bookingIdVal);
          if (!sessionData || !sessionData.order_id) {
            throw new Error("Failed to initialize Razorpay payment order.");
          }
          Alert.alert("Proceed to Payment", "Please complete your 10% advance deposit via Razorpay.", [
            { text: "OK", onPress: () => router.replace('/(user)/bookings') }
          ]);
        } catch (sessionErr) {
          Alert.alert("Payment Error", sessionErr.message || "Failed to generate Razorpay payment order.");
        }
        return;
