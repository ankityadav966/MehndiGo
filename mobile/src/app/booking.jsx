...
import { ChevronLeft, Calendar as CalendarIcon, Clock, CreditCard, Banknote } from 'lucide-react-native';
...
export default function BookingScreen() {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      // If payment is ONLINE, we'd normally trigger Razorpay here.
      // Since Razorpay requires a native SDK integration in Expo, we simulate the success for this migration demo unless requested otherwise.
      if (paymentMethod === "ONLINE") {
        Alert.alert("Payment Simulation", "Simulating Razorpay Payment Success...");
        await bookingService.verifyPayment({
          razorpay_payment_id: "pay_simulated_" + Date.now(),
          razorpay_order_id: res.data.order_id || "order_simulated",
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 12, marginBottom: 32 }}>
          ...
          <TouchableOpacity
            style={[
              styles.glassPanel,
              { flex: 1, alignItems: 'center', borderWidth: 2, borderColor: paymentMethod === 'CASH_ON_DELIVERY' ? colors.accent : 'transparent' }
            ]}
            onPress={() => setPaymentMethod('CASH_ON_DELIVERY')}
          >
            <Banknote size={24} color={paymentMethod === 'CASH_ON_DELIVERY' ? colors.accent : colors.textSecondary} style={{ marginBottom: 8 }} />
            <Text style={{ color: paymentMethod === 'CASH_ON_DELIVERY' ? colors.accent : colors.textSecondary, fontWeight: '600' }}>Cash on Delivery</Text>
          </TouchableOpacity>
        </View>
