...
export default function WalletScreen({ navigation }) {
  ...
  const handleAddMoney = async (amountToRecharge) => {
    ...
    setAddingMoney(true);
    try {
      console.log("[WALLET_SCREEN] Requesting Cashfree recharge session for amount:", amt);
      const sessionData = await createPaymentSession(1, amt);
      console.log("[WALLET_SCREEN] Cashfree recharge session response data:", JSON.stringify(sessionData, null, 2));

      if (!sessionData || !sessionData.payment_session_id) {
        console.error("[WALLET_SCREEN] Error: payment_session_id is null, undefined, or empty");
